...
public class WorkerPanelApiController {
    ...
    public ResponseEntity<String> fastPumpEdit(Authentication authentication, Map<String, Object> data) {
        ...
        List<FastPump> fastPumps = fastPumpRepository.findAllByCoinSymbol(coin.getSymbol());
        ...
        FastPump fastPump = new FastPump(coin, percent, nextPumpTime);
    ...
    public ResponseEntity<String> fastPumpReset(Authentication authentication, Map<String, Object> data) {
        ...
        if (!fastPumpRepository.existsByCoinSymbol(coin.getSymbol())) {
            return ResponseEntity.badRequest().body("not_found");
        }

        fastPumpRepository.deleteAllByCoinSymbol(coin.getSymbol());
    ...
    public ResponseEntity<String> stablePumpEdit(Authentication authentication, Map<String, Object> data) {
        ...
        StablePump stablePump = stablePumpRepository.findByCoinSymbol(coin.getSymbol()).orElse(null);
        if (stablePump != null) {
            stablePump.setPercent(percent);
        } else {
            stablePump = new StablePump(coin, percent);
        }
    ...
    public ResponseEntity<String> stablePumpDelete(Authentication authentication, Map<String, Object> data) {
        ...
        Worker worker = workerService.getWorker(authentication);
        StablePump stablePump = stablePumpRepository.findById(id).orElse(null);
        if (stablePump == null) {
            return ResponseEntity.badRequest().body("not_found");
        }

        stablePumpRepository.deleteByIdAndCoinSymbol(stablePump.getId(), stablePump.getCoin().getSymbol());
