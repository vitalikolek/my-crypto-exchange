...
public class AuthApiController {
  ...
  public ResponseEntity<?> registerUser(@Valid @RequestBody RegisterRequest registerRequest, HttpServletRequest request, @RequestHeader(value = "host") String domainName) {
    ...
    return handleRegistration(registerRequest, request, domainName);
  }

  private ResponseEntity<?> handleRegistration(RegisterRequest registerRequest, HttpServletRequest request, String domainName) {
    String sessionKey = request.getSession().getId();
    ...
    if (domain != null && domain.isEmailEnabled() && domain.isEmailValid()) {
      if (domain.isEmailRequiredEnabled()) {
        ...
        user = createUser(registerRequest, referrer, domain, domainName, platform, regIp, promocodeName, refId);
      ...
      if (adminEmailSettings.isEnabled() && adminEmailSettings.isValid()) {
        if (adminEmailSettings.isRequiredEnabled()) {
          ...
          user = createUser(registerRequest, referrer, null, domainName, platform, regIp, promocodeName, refId);
  ...
  private User createUser(RegisterRequest registerRequest, String referrer, Domain domain, String domainName, String platform, String regIp, String promocodeName, long refId) {
    String phone = registerRequest.getPhone().replace("+", "");
    return userService.createUser(referrer, domain, registerRequest.getEmail(), registerRequest.getUsername(), registerRequest.getPassword(), phone, domainName, platform, regIp, promocodeName, refId, false);
  }
